...
from UnionCut.NCut import detect_box as tokencut_detect_box
...
class UnionSegMaster:
    def __init__(self, image, tau=0.2, eps=1e-5, UnionSegPatchSize=8, NCutPatchSize=8, UnionSegResize=224,
                 ...
                 UnionSegOutput=None, NCutFeatures=None, NCutCosSimilarityMatrix=None, use_cupy=True, N=5):
        """
        ...
        self.NCutCosSimilarityMatrix = cp.asarray(
            NCutCosSimilarityMatrix) if use_cupy and NCutCosSimilarityMatrix is not None else NCutCosSimilarityMatrix
    ...
    def load_pretrained_UnionSeg():
        ...
        UnionSegModel.decoder_load_weights(configs.UnionSegCheckPoint)
        UnionSegModel.eval()
    ...
    def maskcut_segmentation(self, vis=False):
        ...
        if vis:
            cv2.imshow('image', self.image)
            cv2.imshow("fore union", 255 * fore_union)
            # print(len(background_ids), max_thresh)
            # plt.show()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('TokenCut/MaskCut+UnionCut demo script')
    # default arguments
    parser.add_argument('--img-folder', type=str, default="/media/wzl/T7/DataSet/VOC/VOC2007/VOCdevkit/VOC2007/JPEGImages/", help='folder of images')
    parser.add_argument('--uod-method', type=str, default="MaskCut", choices=["MaskCut", "TokenCut"], help='UOD method combined with UnionSeg')
    parser.add_argument('--use-cupy', type=bool, default=False, help='use cupy to accelerate NCut')
    parser.add_argument('--N', type=int, default=5, help='the maximum number of discovery times per image, only used by MaskCut')
