...
from NCut import detect_box as tokencut_detect_box
...
class UnionSegMaster:
    ...
    def _preprocess(image, patch_size, resize, feat_row, feat_col):
        ...
        if resize is not None:
            ...
            RGB_image = cv2.resize(RGB_image, (resize, resize))
        else:
    ...
    def _TokenCut(self, features, fore_union, left_indices, mode="zero"):
        ...
        if mode == "remove":
            ...
            for _ in range(self.N):
                # NCut
                try:
                    bipartition, seed = ncut(cos_similarity_matrix, self.tau, self.eps, back_hard=True, fore_hard=True,
                                             feat_row=self.NCut_feat_row, feat_col=self.NCut_feat_col,
                                             img_row=self.NCut_feat_row * self.NCutPatchSize,
                                             img_col=self.NCut_feat_col * self.NCutPatchSize,
                ...
                # mask out the mask
                cos_similarity_matrix[np.arange(features.shape[0])[bipartition[left_indices]], :] = 0
                cos_similarity_matrix[:, np.arange(features.shape[0])[bipartition[left_indices]]] = 0
            ...
            _, cos_similarity_matrix = self._square_distance_matrix(self._norm_features(features),
                                                                    ...
                                                                    feat_nums=features.shape[0])
            # mask out the mask
            cos_similarity_matrix[np.arange(features.shape[0])[~left_indices], :] = 0
            cos_similarity_matrix[:, np.arange(features.shape[0])[~left_indices]] = 0
            # TokenCut
            for _ in range(self.N):
                # NCut
                ...
                if len(waitlist) == 0:
                    ...
                    bipartition = (sub_fore_mask == 1).reshape(-1)
                    if IoU(sub_fore_mask, waitlist[-1]) > 0.5 or Precision(sub_fore_mask, fore_union) < self.theta:
                        print("IoU: ", IoU(sub_fore_mask, waitlist[-1]), " precision: ",
                              Precision(sub_fore_mask, fore_union))
                        break
                    # remove previous mask from the current mask
                    for mask in waitlist:
                        if Precision(mask, sub_fore_mask) > 0.5:
                ...
                waitlist.append(sub_fore_mask)
                # mask out the mask
                cos_similarity_matrix[np.arange(features.shape[0])[bipartition], :] = 0
                cos_similarity_matrix[:, np.arange(features.shape[0])[bipartition]] = 0
                # judge if the bipartition should be finished
                # calculate the overall segmented mask
                overall_segmented_mask = np.sum(np.stack(waitlist), axis=0)
    ...
    def returnUnion(self, vis=False, resize=False, crf=False):
        ...
        if crf:
            ...
            fore_union = densecrf(I_new, fore_union)
            fore_union = ndimage.binary_fill_holes(fore_union >= 0.5)
        return fore_union
