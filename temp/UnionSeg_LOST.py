...
import os
import scipy
from GraphCut.UnionSeg.inference import UnionSegMaster
...
def LOST_UnionSeg(net, unionseg, image, which_features="k", k=100, crf=True):
    """
    ...
    fore_union = unionseg.returnUnion()
    ...
    pred, _, _, _, raw_mask = lost_unionseg(fs, fore_union, (feat_row, feat_col), (16, 16), (3, image.shape[0], image.shape[1]), k_patches=k)
    mask = cv2.resize(raw_mask, (feat_col * 16, feat_row * 16), cv2.INTER_NEAREST)[: image.shape[0], : image.shape[1]]
    if crf:
        # apply crf
        crf_mask = densecrf(image, mask.astype(np.uint8)).astype(np.uint8)
        if np.max(crf_mask) != 0:
            mask = crf_mask
    return pred, mask


def lost_unionseg(feats, fore_union, dims, scales, init_image_size, k_patches=100):
    """
...
if __name__ == "__main__":
    ...
    for image_path in image_paths[:]:
        ...
        FeatureMaster = UnionSegMaster(image, tau=0.15, eps=1e-5, UnionSegPatchSize=8, NCutPatchSize=8,
                                       ...
                                       pretrained_UnionSeg=UnionSegInstance, use_cupy=False)
        # image = cv2.imread("./CMS_livingroom.PNG")
        # obtain mask
        pred, mask = LOST_UnionSeg(net, FeatureMaster, image)
        cv2.rectangle(
            image,
