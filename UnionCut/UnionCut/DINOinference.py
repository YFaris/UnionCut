...
from  UnionCut.NCut import ncut
...
class Edge:
    ...
    def calculateWeight(self, beta, square_distance_matrix, fore_seed_index=None, back_indices=None, K=None):
        ...
        if self._from.isTerminal is not True and self.to.isTerminal is not True:
            ...
            self.weight = 1 * math.exp(-square_distance_matrix[relative2absolute((self._from.row, self._from.col)), \
...
class GraphCutMaster:
    def __init__(self, image, scan_distance_mode="Euclidean", tau=0.2, eps=1e-5):
        """
        integrate curve drawing and segmentation together
        fine_mode: "OPTICS" or "GraphBased" or None or NCut
        ...
        self.square_distance_matrix, self.correlation_matrix = self._square_distance_matrix(self._norm_features(self.ks[1:, :]), distance_mode=scan_distance_mode)
        # obtain the size of the image
        self.row = image.shape[0]
        self.col = image.shape[1]

    @staticmethod
    def _DINO_features(image):
        """
        ...
        def _load_pretrained_DINO():
            ...
            # net.cuda()
            utils.load_pretrained_weights(net, "/media/wzl/T7/PhysicalImageAugmentation/LOST/dino_deitsmall8_pretrain.pth", checkpoint_key=None,
        ...
        return features.squeeze(0).detach().cpu(), \
                qs.permute(0, 2, 1, 3).reshape(1, 785, -1).squeeze(0).detach().cpu(), \
                ks.permute(0, 2, 1, 3).reshape(1, 785, -1).squeeze(0).detach().cpu(), \
                vs.permute(0, 2, 1, 3).reshape(1, 785, -1).squeeze(0).detach().cpu()
    ...
    @staticmethod
    def _square_distance_matrix(features, distance_mode):
        """
    ...
    def segmentation(self, integrity=True, small_thresh=0.01, vis=True, N=3):
        """
        ...
        norm_accumulated_attention = 255 - (255 * (accumulated_attention - np.min(accumulated_attention)) / (np.max(accumulated_attention) - np.min(accumulated_attention))).astype(np.uint8)
        ...
        fore_combined_mask = np.zeros((28, 28), dtype=np.uint8)
        fore_combined_mask[indices == 0] = 1
        fore_combined_mask[indices == 1] = 0
        # reverse the mask if current foreground covering 4 corners
        if self._check_num_fg_corners(densecrf(self.image, fore_combined_mask)) >= 4 or self._check_num_fg_corners(fore_combined_mask) >= 4:
            indices = 1 - indices
            fore_combined_mask[indices == 0] = 1
            fore_combined_mask[indices == 1] = 0

        # hierarchical NCut
        cos_similarity_matrix = self.correlation_matrix.copy()
        # initialize results list
        masks = []
        for i in range(N):
            # NCut
            try:
                bipartition = ncut(cos_similarity_matrix, self.tau, self.eps, back_hard=True, fore_hard=True, mode="TokenCut")
            except:
                break
            # obtain the abs indices the subsub-region
            sub_fore_indices, sub_back_indices = np.arange(28*28)[bipartition], np.arange(28*28)[~bipartition]
            # transfer sbs indices to relative indices
            sub_fore_relative_indices = self._absolute2relative(sub_fore_indices)
            sub_back_relative_indices = self._absolute2relative(sub_back_indices)
            # obtain mask
            sub_fore_mask, sub_back_mask = np.zeros((28, 28), dtype=np.uint8), np.zeros((28, 28), dtype=np.uint8)
        ...
        if vis:
            ...
            cv2.imshow("attention", cv2.resize(norm_accumulated_attention, (self.col, self.row)))
            cv2.imshow("fore total", 255 * fore_combined_mask)
            print(len(background_ids), max_thresh)
            plt.show()
            cv2.imshow("masks", self._visualization(masks))
            cv2.waitKey(0)
    ...
    def returnUnion(self, vis=True):
        ...
        fore_combined_mask[indices == 1] = 0
        # reverse the mask if current foreground covering 4 corners
        if self._check_num_fg_corners(densecrf(self.image, fore_combined_mask)) >= 4 or self._check_num_fg_corners(
        ...
        if vis:
            cv2.imshow('image', image)
            cv2.imshow("fore total", cv2.resize(255 * fore_combined_mask, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST))
            cv2.imshow("crf total", densecrf(self.image, fore_combined_mask))
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        return fore_combined_mask

def TokenGraphCut(image, integrity=True, scan_distance_mode="Euclidean", small_thresh=0.01, vis=False):
    """
    implementation of TokenCut + UnionCut
    ...
    """
    # initialize graph cut
    graph_cut = GraphCutMaster(image, scan_distance_mode=scan_distance_mode)
    masks = graph_cut.segmentation(integrity=integrity, small_thresh=small_thresh, vis=vis)
    return masks
