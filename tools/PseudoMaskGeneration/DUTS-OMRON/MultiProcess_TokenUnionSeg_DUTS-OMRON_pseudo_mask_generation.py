...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), "UnionSeg"))
import argparse
from inference import UnionSegMaster
...
def h5writer(result_queue, patch_size=16, resize_mode="flexible", UnionCut_patch_size=8, UnionCut_resize_mode="fixed"):
    # create the h5 file
    file = h5py.File("./h5/TokenUnionSegN1_tao0.2_DUTS-OMRON_ViT_S_" + str(patch_size) + "_" + resize_mode + "_" + str(UnionCut_patch_size) + "_" + UnionCut_resize_mode +".h5", "w")
    # create a group to save pseudo-labels generated by the current method
    method_group = file.create_group("UnionSeg")
...
def mask_generation(result_queue, image_name, local_path, image_path):
    ...
    fore_union = TokenUnionSeg.returnUnion()
    masks, _ = TokenUnionSeg.tokencut_segmentation()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('Saving TokenCut+UnionSeg results to h5')
    # default arguments
    parser.add_argument('--dataset', type=str, default="/home/wzl/DataSet/", help='root path of the DUTS-OMRON dataset')
    ...
    image_names = [image.split('.')[0] for image in
                   os.listdir(root_path + "DUT-OMRON/DUT-OMRON-image/DUT-OMRON-image/")]

    # save absolute paths of each image in the computer
    local_paths = [root_path + "DUT-OMRON/DUT-OMRON-image/DUT-OMRON-image/" + image_name + ".jpg"
                   for image_name in image_names]
    # save relevant paths of each image under the dataset
    image_paths = ["DUT-OMRON/DUT-OMRON-image/DUT-OMRON-image/" + image_name + ".jpg" for image_name in image_names]
